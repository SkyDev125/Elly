...
if [ "$MAP_TYPE" == "2d" ]; then
    ...
    echo "--- Starting Remote Localization & Navigation (2D) ---"
    ssh -t $NANO_USER@$NANO_IP "source /opt/ros/galactic/setup.bash && source ~/myagv_ros2/install/setup.bash && ros2 launch nav2_bringup bringup_launch.py map:=$REMOTE_MAP_DIR/$MAP_NAME.yaml params_file:=$REMOTE_CONFIG_DIR/myagv_nav2.yaml use_sim_time:=false"
    ...
    ssh -t $NANO_USER@$NANO_IP "source /opt/ros/galactic/setup.bash && source ~/myagv_ros2/install/setup.bash && source ~/ros2_astra_ws/install/setup.bash && $CMD_LOC"
